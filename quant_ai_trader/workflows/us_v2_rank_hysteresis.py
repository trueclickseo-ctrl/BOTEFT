"""One-variable v2 test: top-8 entry and top-10 incumbent retention."""
...
def _evaluate(bars, buffer, stress=False):
    config = DefensiveMomentumConfig(
        holdings=8, target_annual_volatility=.10, trend_lookback_days=150,
        risk_adjusted_momentum_ranking=True, rank_hysteresis_buffer=buffer,
...
def run():
    ...
    output = {}
    for name, buffer in (("risk_adjusted_baseline", 0), ("top_8_enter_top_10_retain", 2)):
        curve, decisions, metrics = _evaluate(bars, buffer)
        _, _, stress = _evaluate(bars, buffer, stress=True)
        ...
        output[name] = {"metrics": metrics, "fold_returns": folds, "positive_folds": positive,
                        ...
                        "rank_exits": int(decisions.rank_exits.sum())}
    return output
