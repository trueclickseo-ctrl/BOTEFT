"""Evaluate an unlevered, volatility-targeted dual-momentum portfolio."""
from __future__ import annotations
import argparse
from datetime import UTC, datetime
from uuid import uuid4
from quant_ai_trader.backtesting.dual_momentum import run_equal_weight_backtest, run_risk_targeted_dual_momentum_backtest
...
def run(symbols: list[str]) -> dict[str, float]:
    ...
    _, decisions, metrics = run_risk_targeted_dual_momentum_backtest(frames)
    ...
    metrics["beats_equal_weight"] = float(metrics["risk_gate_passed"] and metrics["total_return"] > benchmark["total_return"] and metrics["sharpe_ratio"] > benchmark["sharpe_ratio"])
    metrics["research_only"] = 1.0
    repo.record_strategy_run(str(uuid4()), "risk_targeted_dual_momentum_v1", ",".join(normalized), datetime.now(UTC).isoformat(), metrics)
    return metrics
