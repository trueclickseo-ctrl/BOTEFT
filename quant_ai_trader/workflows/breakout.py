"""Evaluate and log the ATR breakout strategy."""
import argparse
...
def run(symbol: str):
    ...
    result = ETFBacktester().run(bars, breakout_signals(build_feature_dataset(bars, spy_bars=spy)))
    repo.record_strategy_run(str(uuid4()), "atr_breakout_v1", symbol.upper(), datetime.now(UTC).isoformat(), result.metrics)
