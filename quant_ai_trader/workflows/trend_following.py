"""Evaluate and record the trend-following strategy."""
import argparse
...
def run(symbol: str) -> dict[str, float]:
    ...
    result = ETFBacktester().run(bars, trend_following_signals(build_feature_dataset(bars, spy_bars=spy)))
    repo.record_strategy_run(str(uuid4()), "trend_following_v1", symbol.upper(), datetime.now(UTC).isoformat(), result.metrics)
